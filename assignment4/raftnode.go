...
func New(config Config) *RaftNode {
	...
	var votefor int
	var term int
	file, e := ioutil.ReadFile(strconv.Itoa(config.Id) + "state.json")
	if e != nil {
	...
	for logLastIndex >= 0 {
		...
		if err == nil {
			...
			if ok {
				...
				LogArray = append(LogArray, LogInfo{Term: foo.Term, Data: foo.Data})
			}
		}
		startindex++
		logLastIndex--
	...
	//rNode.nodeLog.Close()
	//fmt.Println("log LogArray", LogArray)
	//initialize state machine instance
	s := &Server{MyId: config.Id, CurrentTerm: term, VotedFor: votefor, Log: LogArray, CommitIndex: -1, Leader: 0, State: "FOLLOWER", Peers: peerArray, VoteGranted: voteArray, NextIndex: nextInd, MatchIndex: matchInd, PersistantLog: lg}
	...
	if config.Id == 1 {
		rNode.Timer = time.AfterFunc(time.Duration(config.ElectionTimeout)*time.Millisecond, func() { rNode.timeoutCh <- Timeout{} })
	} else {
...
//initialize single raft
func makeSingleRafts(rNode *RaftNode) *RaftNode {

	//initialize peer config file
	cl := rNode.cluster
	cl.AddServer(rNode.ID())
	...
	var votefor int
	var term int
	file, e := ioutil.ReadFile(strconv.Itoa(rNode.ID()) + "state.json")
	if e != nil {
		fmt.Printf("File error: %v\n", e)
	}
	...
	//intialize logs for the node
	//fmt.Println("log dir",config.LogDir)
	lg, err := log.Open("NodeLog" + strconv.Itoa(rNode.ID()))
	if err != nil {
		fmt.Println("Error : Log Open() failed", err)
	...
	fmt.Println("log LogArray", LogArray)
	//initialize state machine instance
	s := &Server{MyId: rNode.ID(), CurrentTerm: term, VotedFor: votefor, Log: LogArray, CommitIndex: -1, Leader: 0, State: "FOLLOWER", Peers: peerArray, VoteGranted: voteArray, NextIndex: nextInd, MatchIndex: matchInd, PersistantLog: lg}
	rNode.StateMachine = s
	// Give each raftNode its own "Server" from the cluster.
	rNode.cluster_Server = rNode.cluster.Servers[rNode.ID()]
	//rNode.cluster = config.config_MockCluster
	...
	//set election timeout
	if rNode.ID() == 1 {
		rNode.Timer = time.AfterFunc(time.Duration(500)*time.Millisecond, func() { rNode.timeoutCh <- Timeout{} })
	} else {
		rNode.Timer = time.AfterFunc(time.Duration(500+RandNo)*time.Millisecond, func() { rNode.timeoutCh <- Timeout{} })
	}

	//set heartbeat timer
	rNode.HeartbeatTimer = time.NewTicker(time.Duration(150) * time.Millisecond)
...
func (rn *RaftNode) Append(Data DataStore) {
...
func makeRafts() []*RaftNode {
	...
	configuration := Config{Id: 1, LogDir: "./NodeLog1", ElectionTimeout: 500, HeartbeatTimeout: 150, config_MockCluster: cl}
	...
	configuration = Config{Id: 2, LogDir: "./NodeLog2", ElectionTimeout: 500, HeartbeatTimeout: 150, config_MockCluster: cl}
	result2 := New(configuration)
	go result2.ProcessEvents()
	node = append(node, result2)
	configuration = Config{Id: 3, LogDir: "./NodeLog3", ElectionTimeout: 500, HeartbeatTimeout: 150, config_MockCluster: cl}
	result3 := New(configuration)
	go result3.ProcessEvents()
	node = append(node, result3)
	configuration = Config{Id: 4, LogDir: "./NodeLog4", ElectionTimeout: 500, HeartbeatTimeout: 150, config_MockCluster: cl}
	result4 := New(configuration)
	go result4.ProcessEvents()
	node = append(node, result4)
	configuration = Config{Id: 5, LogDir: "./NodeLog5", ElectionTimeout: 500, HeartbeatTimeout: 150, config_MockCluster: cl}
	...
	for {
		...
		case ev = <-rn.eventCh:
			/*if ev != nil && ev != "" {
				...
				fmt.Printf("batch requests %+v", batch)
			}
			if len(batch) >= 10 {
				rn.SendBatch(batch)
				batch = batch[:0]
		...
		case _, ok := <-rn.shutdownch:
			if !ok {
				...
				rn.StateMachine.Log = make([]LogInfo, 0)
				close(rn.CommitChan)
				close(rn.timeoutCh)
				close(rn.eventCh)
				wg.Done()
...
/*func (rn *RaftNode) SendBatch(batch []Event) {
	actions := make([]Action, 0)
	fmt.Printf("batch requests %+v", batch)
	fmt.Println("...")
	for i := 0; i < len(batch); i++ {
		if batch[i] != nil && batch[i] != "" {
			fmt.Printf("sequential batch  %+v", batch[i])
			actions = rn.StateMachine.ProcessEvent(batch[i])
		}
...
func (rn *RaftNode) DoActions(actions []Action) {
	for _, action := range actions {
		...
		case Send:
			...
			if rn.StateMachine.State != "LEADER" {
				rn.Timer.Stop()
				rn.Timer = time.AfterFunc(time.Duration(800+rand.Intn(250))*time.Millisecond, func() { rn.timeoutCh <- Timeout{} })
			}
			obj := action.(Send)
		...
		case Commit:
			obj := action.(Commit)
			//fmt.Printf("commit %+v id : %d", obj, rn.ID())
			rn.CommitChan <- &obj
		...
		case StateStore:
			...
			//rn.myLock.Lock()
			//fmt.Println(strconv.Itoa(rn.ID())+"state.json")
			file, e := ioutil.ReadFile(strconv.Itoa(rn.ID()) + "state.json")
			//myLock.Unlock()
			if e != nil {
				fmt.Printf("File error: %v\n", e)
			}
			...
			var objArray []PersistantStore
			json.Unmarshal(file, &objArray)
			//fmt.Println("statestore",rn.ID(),objArray)

			objArray[0].Term = term
			objArray[0].VotedFor = votefor

			fileJson, _ := json.Marshal(objArray)

			err := ioutil.WriteFile(strconv.Itoa(rn.ID())+"state.json", fileJson, 0644)
			//fmt.Println("statestore",rn.ID(),objArray)
			//rn.myLock.Unlock()
			if err != nil {
			...
			fmt.Println("log store", action, " id", rn.ID())
