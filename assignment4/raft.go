...
func OnVoteReqFollower(s *Server, msg VoteReq) []Action {
	...
	//LogIndex := len(s.Log)
	LogIndex1 := s.PersistantLog.GetLastIndex()
	LogIndex := int(LogIndex1)
	LogTerm := 0
	if LogIndex >= 0 {
		//get last Term From Logs
		data, err := s.PersistantLog.Get(LogIndex1)
		if err == nil {
			record, ok := data.(LogStore)
...
func OnAppendEntriesReqFollower(s *Server, msg AppendEntriesReq) []Action {
	actionArray := make([]Action, 0)
	LogLength := int(s.PersistantLog.GetLastIndex()) + 1
	index := LogLength - 1
	term := 0
	if index >= 0 {
		//get last Term From Logs
		data, err := s.PersistantLog.Get(int64(index))
		if err == nil {
			record, ok := data.(LogStore)
	...
	if s.CurrentTerm > msg.Term {
		...
		actionArray = append(actionArray, Send{From: msg.LeaderId, Event: AppendEntriesResp{Term: s.CurrentTerm, Success: false, From: s.MyId, Count: len(msg.Entries), LastLogIndex: msg.LastLogIndex}})
	} else if msg.LastLogIndex != -1 && LogLength != 0 && term != msg.LastLogTerm {
		...
		if len(msg.Entries) > 0 {
			...
			if LogLength > i {
				...
				s.PersistantLog.TruncateToEnd(int64(i))
			}
			if len(msg.Entries) > 0 {
				...
				var store LogStore
				if latestTerm > s.CurrentTerm {
					actionArray = append(actionArray, LogStore{Index: i, Data: msg.Entries[0].Data, Term: latestTerm})
					store = LogStore{Index: i, Data: msg.Entries[0].Data, Term: latestTerm}
				} else {
					actionArray = append(actionArray, LogStore{Index: i, Data: msg.Entries[0].Data, Term: s.CurrentTerm})
					store = LogStore{Index: i, Data: msg.Entries[0].Data, Term: s.CurrentTerm}
				}

				//s.Log = append(s.Log, msg.Entries...)
				s.PersistantLog.Append(store)
			}
		}
		//set Leader id
		s.Leader = msg.LeaderId
		//fmt.Println("Leader number-----------", s.Leader, msg.LeaderId)
		...
		if msg.LeaderCommit > s.CommitIndex {
			...
			data, err := s.PersistantLog.Get(int64(s.CommitIndex))
			if err == nil {
				record, ok := data.(LogStore)
				...
				if ok {
					...
					actionArray = append(actionArray, Commit{Index: s.CommitIndex, Data: storedData})
				}
			}
...
func onTimeOutFollower(s *Server) []Action {
	...
	if logLength > 0 {
		...
		//get last Term From Logs
		data, err := s.PersistantLog.Get(int64(lastIndex))
		if err == nil {
			record, ok := data.(LogStore)
...
func OnAppendRequestFromClientToFollower(s *Server, msg Append) []Action {
	actionArray := make([]Action, 0)
	actionArray = append(actionArray, Commit{Data: msg.Data, Error: strconv.Itoa(s.Leader)})
	return actionArray
}
...
func OnVoteReqCandidate(s *Server, msg VoteReq) []Action {
	actionArray := make([]Action, 0)
	LogIndex := int(s.PersistantLog.GetLastIndex()) + 1
	LogTerm := 0
	if LogIndex > 0 {
		//get last Term From Logs
		data, err := s.PersistantLog.Get(int64(LogIndex))
		if err == nil {
			record, ok := data.(LogStore)
...
func OnVoteRespCandidate(s *Server, msg VoteResp) []Action {
	...
	if yesVoteCount >= neededVotes {
		...
		lastIndex := -1
		lastTerm := -1
		leng := int(s.PersistantLog.GetLastIndex()) + 1
		if leng > 0 {
			lastIndex = leng - 1
			//get last Term From Logs
			data, err := s.PersistantLog.Get(int64(lastIndex))
...
func OnVoteReqLeader(s *Server, msg VoteReq) []Action {
	...
	if LogIndex > 0 {
		//get last Term From Logs
		//get last Term From Logs
		data, err := s.PersistantLog.Get(int64(LogIndex - 1))
		if err == nil {
			record, ok := data.(LogStore)
...
func OnAppendRequestFromClientToLeader(s *Server, msg Append) []Action {
	...
	lastIndex := int(s.PersistantLog.GetLastIndex())
	lastTerm := -1
	if lastIndex >= 0 {
		//get last Term From Logs
		data, err := s.PersistantLog.Get(int64(lastIndex))
	...
	store := LogStore{Index: lastIndex + 1, Data: msg.Data, Term: s.CurrentTerm}
	s.PersistantLog.Append(store)
	//s.Log = append(s.Log, LogInfo{Term: s.CurrentTerm, Data: msg.Data})
	//send Logstore action to raft node
	actionArray = append(actionArray, LogStore{Index: (lastIndex + 1), Data: msg.Data, Term: s.CurrentTerm})
	//set next Index and match Index
	s.NextIndex = make([]int, 10)
	s.MatchIndex = make([]int, 10)
	for i := 0; i < 4; i++ {
...
func OnAppendEntriesRespLeader(s *Server, msg AppendEntriesResp) []Action {
	...
	if s.CurrentTerm < msg.Term {
		...
		if msg.Success == false {
			...
			//restore logs
			length := int(s.PersistantLog.GetLastIndex()) + 1
			//logLastIndex := length - 1
			if length > 0 {
				for lastIndex+1 >= length-1 {
					data, err := s.PersistantLog.Get(int64(lastIndex + 1)) // should return the Foo instance appended above
					if err == nil {
						foo, ok := data.(LogStore)

						if ok {

							Entries = append(Entries, LogInfo{Term: foo.Term, Data: foo.Data})
						}
					}
					lastIndex++
		...
		if N >= 0 {
			//get last Term From Logs
			data, err := s.PersistantLog.Get(int64(N))
			if err == nil {
				record, ok := data.(LogStore)
		...
		if length >= 1 {
			...
			if N > s.CommitIndex && Nterm == s.CurrentTerm {
				//Update new commit Index
				s.CommitIndex = N
				//send commit action to client
				actionArray = append(actionArray, Commit{Index: N, Data: Ndata})
